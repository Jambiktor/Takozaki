...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['confirm_order'])) {
    ...
    $delivered_order = date('Y-m-d H:i:s'); // Get the current date and time

    $sql_update = "UPDATE order_table SET status = 'completed', delivered_date = ? WHERE order_number = ?";
    $stmt_update = $conn->prepare($sql_update);
    $stmt_update->bind_param("si", $delivered_order, $order_number);
    $stmt_update->execute();
    $stmt_update->close();

    header("Location: admin_completed_order.php");
    exit();
}
?>
...
<body>
    ...
    <div class="main_body rounded p-2 my-3">
        <div class="cart_header rounded p-2 gap-1">
            <a class="rounded py-2 " href="admin_pendings_order.php">
    ...
    <div class="cart_container rounded p-2 mt-2">
        <?php
        $query = "SELECT * FROM order_table WHERE status = 'Preparing' GROUP BY order_number ORDER BY order_time"; // Filter by status
